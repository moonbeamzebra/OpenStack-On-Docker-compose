...
openstack endpoint create --region $REGION1 \
  compute public http://$NOVA_HOST:8774/v2.1/%\(tenant_id\)s
openstack endpoint create --region $REGION1 \
  compute internal http://$NOVA_HOST:8774/v2.1/%\(tenant_id\)s
openstack endpoint create --region $REGION1 \
  compute admin http://$NOVA_HOST:8774/v2.1/%\(tenant_id\)s

cp /etc/nova/nova.conf /etc/nova/nova.conf.bak
crudini --set /etc/nova/nova.conf DEFAULT enabled_apis osapi_compute,metadata

crudini --set /etc/nova/nova.conf api_database connection mysql+pymysql://nova:$NOVA_DBPASS@$MYSQLHOST/nova_api
